import { useCallback, useEffect, useState } from 'react';
import {
  saveStudySession,
  getStudySessions,
  saveTask,
  getTasks,
  updateTask as updateTaskStorage,
  deleteTask,
  getBreaks
} from '../utils/storage';

const useStudyData = () => {
  const [sessions, setSessions] = useState([]);
  const [tasks, setTasks] = useState([]);
  const [breaks, setBreaks] = useState([]);
  const [stats, setStats] = useState({
    totalTime: 0,
    sessionsCount: 0,
    streak: 0,
    averageTime: 0
  });

  const updateStats = useCallback((sessionList) => {
    const counted = (sessionList || []).filter((s) => !s?.isExam);
    const totalTime = counted.reduce((sum, s) => sum + (s.duration || 0), 0);
    const sessionsCount = counted.length;
    const averageTime = sessionsCount > 0 ? Math.round(totalTime / sessionsCount) : 0;
    const uniqueDays = new Set(counted.map((s) => new Date(s.date).toDateString()));
    const streak = uniqueDays.size;
    setStats({ totalTime, sessionsCount, streak, averageTime });
  }, []);

  const reloadData = useCallback(() => {
    const loadedSessions = getStudySessions();
    const loadedTasks = getTasks();
    const loadedBreaks = getBreaks();

    const safeSessions = Array.isArray(loadedSessions) ? loadedSessions : [];
    setSessions(safeSessions);
    setTasks(Array.isArray(loadedTasks) ? loadedTasks : []);
    setBreaks(Array.isArray(loadedBreaks) ? loadedBreaks : []);
    updateStats(safeSessions);
  }, [updateStats]);

  useEffect(() => {
    reloadData();
  }, [reloadData]);

  const addSession = useCallback(
    (sessionData) => {
      let subject = 'Session de travail';
      if (sessionData?.subject) {
        if (typeof sessionData.subject === 'string') subject = sessionData.subject;
        else if (typeof sessionData.subject === 'object' && sessionData.subject?.name) subject = sessionData.subject.name;
      }

      const newSession = {
        id: sessionData?.id || `session-${Date.now()}-${Math.random().toString(36).slice(2)}`,
        date: sessionData?.date || new Date().toISOString(),
        subject,
        description: sessionData?.description || '',
        duration: Number(sessionData?.duration || 0),
        startTime: sessionData?.startTime,
        isExam: !!sessionData?.isExam
      };

      saveStudySession(newSession);

      setSessions((prev) => {
        const updated = [...(Array.isArray(prev) ? prev : []), newSession];
        updateStats(updated);
        return updated;
      });

      window.dispatchEvent(new Event('sessionAdded'));
    },
    [updateStats]
  );

  const addMultipleSessions = useCallback(
    (sessionsArray) => {
      const all = getStudySessions();
      const base = Array.isArray(all) ? all : [];

      const newSessions = (sessionsArray || []).map((s) => {
        let subject = 'Session de travail';
        if (s?.subject) {
          if (typeof s.subject === 'string') subject = s.subject;
          else if (typeof s.subject === 'object' && s.subject?.name) subject = s.subject.name;
        }

        return {
          id: s?.id || `session-${Date.now()}-${Math.random().toString(36).slice(2)}`,
          date: s?.date || new Date().toISOString(),
          subject,
          description: s?.description || '',
          duration: Number(s?.duration || 0),
          startTime: s?.startTime,
          isExam: !!s?.isExam
        };
      });

      const updated = [...base, ...newSessions];
      localStorage.setItem('studySessions', JSON.stringify(updated));
      setSessions(updated);
      updateStats(updated);

      window.dispatchEvent(new Event('sessionAdded'));
      return newSessions.length;
    },
    [updateStats]
  );

  const deleteSession = useCallback(
    (sessionId) => {
      const all = getStudySessions();
      const base = Array.isArray(all) ? all : [];
      const filtered = base.filter((s) => s.id !== sessionId);

      localStorage.setItem('studySessions', JSON.stringify(filtered));
      setSessions(filtered);
      updateStats(filtered);

      window.dispatchEvent(new Event('sessionAdded'));
    },
    [updateStats]
  );

  const addTask = useCallback((taskData) => {
    const newTask = {
      id: `task-${Date.now()}`,
      completed: false,
      createdAt: new Date().toISOString(),
      ...taskData
    };
    saveTask(newTask);
    setTasks((prev) => [...(Array.isArray(prev) ? prev : []), newTask]);
  }, []);

  const toggleTaskComplete = useCallback((taskId) => {
    setTasks((prev) => {
      const updated = (Array.isArray(prev) ? prev : []).map((t) =>
        t.id === taskId
          ? { ...t, completed: !t.completed, completedAt: !t.completed ? new Date().toISOString() : null }
          : t
      );

      const changed = updated.find((t) => t.id === taskId);
      if (changed) updateTaskStorage(taskId, changed);

      return updated;
    });
  }, []);

  const removeTask = useCallback((taskId) => {
    deleteTask(taskId);
    setTasks((prev) => (Array.isArray(prev) ? prev : []).filter((t) => t.id !== taskId));
  }, []);

  const getTodaySessions = useCallback(() => {
    const today = new Date().toDateString();
    return (Array.isArray(sessions) ? sessions : []).filter(
      (s) => !s?.isExam && new Date(s.date).toDateString() === today
    );
  }, [sessions]);

  const getWeekSessions = useCallback(() => {
    const weekAgo = new Date();
    weekAgo.setDate(weekAgo.getDate() - 7);
    return (Array.isArray(sessions) ? sessions : []).filter((s) => !s?.isExam && new Date(s.date) >= weekAgo);
  }, [sessions]);

  const getTodayBreaks = useCallback(() => {
    const today = new Date().toDateString();
    return (Array.isArray(breaks) ? breaks : []).filter((b) => new Date(b.date).toDateString() === today);
  }, [breaks]);

  return {
    sessions,
    tasks,
    breaks,
    stats,
    addSession,
    addMultipleSessions,
    deleteSession,
    addTask,
    toggleTaskComplete,
    removeTask,
    getTodaySessions,
    getWeekSessions,
    getTodayBreaks,
    reloadData
  };
};

export default useStudyData;
